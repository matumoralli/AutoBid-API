      ...
      items: [
        {
          ...
          title: car.dataValues.brand + " " + car.dataValues.model + ". Subasta ganada por " + buyer.dataValues.name + ". AUTOBID",
...
async function setPayment(req) {
  ...
  try {
    ...
    if (req.query.type === "payment") {
      ...
      const info = await axios.get(URL_WEBHOOK_MERCADOPAGO + req.query["data.id"], {
        headers: {
          Authorization: `Bearer ${ACCESS_TOKEN_MERCADOPAGO}`
        }
      });
      ...
      const payment = await PaymentCredit.findByPk(info.data.additional_info.items[0].id);
...
async function setCreditPayment(req) {
  try {
    ...
    if (req.query.type === "payment") {
      ...
      const paymentCredit = await PaymentCredit.findByPk(info.data.additional_info.items[0].id);
